...
def parse_args():
    ...
    parser.add_argument(
        "--prob-file",
        metavar="prob.json",
        help="Model estimates of probability of all answer.",
    )
    parser.add_argument(
        "--prob-thresh",
        type=float,
        default=0.55,
        help='Predict "" if answer probability is below this (default = 0.55).',
    )
    parser.add_argument(
        "--na-prob-file",
        "-n",
...
def make_qid_to_has_ans(dataset):
    ...
    for article in dataset:
        for p in article["paragraphs"]:
            for qa in p["qas"]:
                id = qa["id"]
                qid_to_has_ans[id] = bool(qa["answers"])
...
def get_raw_scores(dataset, preds, model_probs, thres=0.55):
    exact_scores = {}
    f1_scores = {}
    for article in dataset:
        for p in article["paragraphs"]:
...
def main():
    ...
    if OPTS.prob_file:
        with open(OPTS.prob_file) as f:
            model_probs = json.load(f)
            model_thres  = OPTS.prob_thresh
    else:
        model_probs = {k: 0.0 for k in preds}
        model_thres = -1
    qid_to_has_ans = make_qid_to_has_ans(dataset)  # maps qid to True/False
    has_ans_qids = [k for k, v in qid_to_has_ans.items() if v]
    no_ans_qids = [k for k, v in qid_to_has_ans.items() if not v]
    exact_raw, f1_raw = get_raw_scores(dataset, preds, model_probs, thres=model_thres)
